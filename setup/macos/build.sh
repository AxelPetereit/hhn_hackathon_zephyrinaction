...
#
# Build the Flappy Microchip example for the PIC64GX1000 Curiosity Kit on macOS.
...
# BSD stat differs from GNU coreutils
file_size() { stat -f%z "$1" 2>/dev/null || stat -c%s "$1"; }

WEST="${WORKSPACE}/.venv/bin/west"
APP="${APP_SOURCE}/demos/pic64_smp_hello"

[ -x "${WEST}" ] || die "west not found at ${WEST}. Run setup/macos/setup.sh first."
[ -d "${APP}" ] || die "application not found: ${APP}"
[ -d "${WORKSPACE}/zephyr" ] || die "Zephyr tree missing at ${WORKSPACE}/zephyr. Run setup/macos/setup.sh first."
...
Re-run setup/macos/setup.sh to correct the checkout."
fi
...
echo "Next: ./setup/macos/payload.sh"
